...
object Main extends IOApp {
  ...
  override def run(args: List[String]): IO[ExitCode] =
    for {
      _ <- IO.pure(args.nonEmpty).flatMap {
        case true =>
          Pars.service(PlusOne.run)(pe)(StandAloneCoordinator[IO](CoordinatorAddress)).compile.drain
        case false =>
          Pars.service(Stream.eval(IO.never))(pe)().compile.drain
      }
